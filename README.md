...
網頁可以**上傳**、不可以**拍攝**：上傳不是在外面發生的捕捉，是回到座位後的補件與整理，屬於管理
（IG 網頁版就是這條線：能發文、能上傳、沒有相機）。所以 App 的網頁版把「拍名片」改成「收名片」＝
拖放批次上傳，上傳進來的一律標成 `上傳補件`，不冒充現場收的。企業後台不做也不需要掃名片入口。
詳見 `HANDOFF.md` 第 18、20 節。
